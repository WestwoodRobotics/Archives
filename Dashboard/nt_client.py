...
class NetworkTableClient(object):
    def __init__(self, team, debug = False, server = "127.0.0.1"):
    ...
    def interpretEntry(self, data):
        ...
        if self.watches.has_key(name):
            out2 = self.watches[name]
            out2[0](out2[1], self.tableValues[entryId].value)

        return out

    def watch(self, name, _id, func):
        self.watches[name] = [func, _id]
    ...
    def updateEntry(self, data):
        ...
        if entry.getType() == bool:
            ...
            """Double"""
            out = struct.unpack('!d', str(data[0:8]))[0]
            out1 = data[8:]
        elif entry.getType() == str:
            """String"""
            strLen = self.getNumFromBytes(data[0:2])
            out = str(data[2:strLen+2])
            out1 = data[strLen + 2:]

        entry.update(out, seq)
        if self.watches.has_key(entry.name):
            out2 = self.watches[entry.name]
            out2[0](out2[1], entry.value)
